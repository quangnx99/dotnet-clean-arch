...
<#
.SYNOPSIS
    Install the dotnet-clean-arch Claude Code skill (and the dotnet template) in one command.
...
.DESCRIPTION
    ...
    2. Copies the skill file to $HOME\.claude\skills\dotnet-clean-arch\SKILL.md.
    3. Prints next steps.
#>
...
$RepoUrl    = 'https://github.com/quangnx99/dotnet-clean-arch.git'
$SkillSrc   = Join-Path $PSScriptRoot '.claude\skills\dotnet-clean-arch\SKILL.md'
$SkillDest  = Join-Path $HOME '.claude\skills\dotnet-clean-arch\SKILL.md'
...
Write-Host ''
Write-Host 'dotnet-clean-arch skill installer' -ForegroundColor White
...
if ($templateMissing) {
    ...
    $TempDir = Join-Path ([System.IO.Path]::GetTempPath()) "dotnet-clean-arch-$([System.IO.Path]::GetRandomFileName())"
...
Write-Host '       /dotnet-clean-arch' -ForegroundColor Yellow
